...
class OwnerCog(commands.Cog, name="Owner"):
    ...
    async def add_dev(self, ctx):
        ...
        self.bot.pyMongoManager.collection_guilds.find_one_and_update(
            {"guild_id": 512830421805826048}, {"$push": {"developers": int(args[0])}}
        )
    ...
    async def delete_dev(self, ctx):
        if ctx.author.id not in self.bot.pyMongoManager.get_dev_list():
            return
        args = ctx.message.content.split()[1:]
        panchessco = self.bot.pyMongoManager.collection_guilds.find_one(
            {"guild_id": 512830421805826048}
        )
        panchessco["developers"].remove(int(args[0]))
        self.bot.pyMongoManager.collection_guilds.replace_one(
            {"guild_id": 512830421805826048}, panchessco
        )
    ...
    async def unbelievaboat(self, ctx):
        ...
        if ctx.author.id not in staff_list:
            return
        await ctx.send(
            "¿Estás seguro? (Este proceso es irreversible y modificará el dinero de todos los usuarios) [Y/n]"
        )
        try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://unbelievaboat.com/api/v1/guilds/512830421805826048/leaderboard?limit=25&page=1"
            ) as resp:
                if resp.status == 200:
        ...
        for x in range(pages):
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://unbelievaboat.com/api/v1/guilds/512830421805826048/leaderboard?limit=25&page={x+2}"
                ) as resp:
                    if resp.status == 200:
                        data_json = await resp.json()
        ...
        if spend_time / 60 >= 1:
            ...
            m = 0
        await ctx.send(
            f"Tarea completada, tiempo empleado aproximado: `{int(round(m, 0))}m y {int(round(s, 0))}`s"
        )
