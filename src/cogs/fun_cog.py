...
class EmojiCog(commands.Cog, name="Emoji"):
    def __init__(self, bot):
        ...
        self.star_board_dict = (
            {}
        )  # original_message_id: { startbaord_channel_message_id, emoji_id}
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.message_id in autoReactionMenuStackBean.getMessageIdList():
            await autoReactionMenuStackBean.updateMenuBean(
                payload.message_id, payload.user_id, payload.emoji
            )
        elif payload.message_id in autoReplyMenuStackBean.getMessageIdList():
            await autoReplyMenuStackBean.updateMenuBean(
                payload.message_id, payload.user_id, payload.emoji
            )
            ...
            if category_channel.id in config.star_board_category_channels_id:
                if payload.message_id in self.star_board_dict.keys():
                    ...
                    if payload.emoji.id and payload.emoji.id == star_board["emoji_id"]:
                        channel_star_board = self.bot.get_channel(
                            config.channel_star_board_id
                        )
                        msg_channel_star_board = await channel_star_board.fetch_message(
                            star_board["starboard_channel_message_id"]
                        )

                        count = int(msg_channel_star_board.content.split("**")[1])

                        new_content = msg_channel_star_board.content.replace(
                            f"**{count}**", f"**{count+1}**"
                        )
                    ...
                    original_message = await channel_message.fetch_message(
                        payload.message_id
                    )

                    content_lower = unidecode.unidecode(
                        original_message.content.lower().replace(".", "")
                    )
                    ...
                    if (
                        payload.emoji.id
                        and self.bot.get_emoji(payload.emoji.id)
                        and not original_message.author.bot
                    ):
                        emoji_count = 0
                        ...
                        for reaction in original_message.reactions:
                            if (
                                ...
                                and reaction.emoji.id == payload.emoji.id
                            ):
                        ...
                        if emoji_count >= self.reaction_count_min:
                            ...
                            embed_star_board.colour = discord.Colour.from_rgb(
                                255, 215, 0
                            )
                            author = original_message.author
                            embed_star_board.set_author(
                                name=author.name, icon_url=author.avatar_url
                            )
                            ...
                            if len(original_message.attachments) > 0:
                                if (
                                    "image"
                                    in original_message.attachments[0].content_type
                                ):
                                    embed_star_board.set_image(
                                        url=original_message.attachments[0].url
                                    )

                            embed_star_board.add_field(
                                name="Fuente",
                            ...
                            msg_channel_star_board = await channel_star_board.send(
                                content=star_board_message_content,
                                embed=embed_star_board,
                            )
                            await msg_channel_star_board.add_reaction(payload.emoji)

                            self.star_board_dict[original_message.id][
                                "starboard_channel_message_id"
                            ] = msg_channel_star_board.id

                            self.bot.pyMongoManager.add_star_board(
                                original_message.id,
                                msg_channel_star_board.id,
                                payload.emoji.id,
                            )
    ...
    async def addautoreaction(self, ctx):
        ...
        if len(args) < 2:
            ...
            if ctx.author.id in self.bot.get_panchessco_staff_id_list():
                ...
                await ctx.send(
                    "Solo administradores de panchessco pueden usar el comando"
                )
    ...
    async def autoreactionlist(self, ctx):
        ...
        autoReactionMenuBean = AutoReactionMenuBean(
            autoReactionList,
            self.bot.get_embed_color(ctx.author.id),
            ctx.author.id,
            self.bot,
            10,
            page,
    ...
    async def autoreplylist(self, ctx):
        ...
        autoReplyMenuBean = AutoReplyMenuBean(
            autoReplyList,
            self.bot.get_embed_color(ctx.author.id),
            ctx.author.id,
            self.bot,
