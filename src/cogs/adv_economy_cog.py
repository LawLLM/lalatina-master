...
class AdvEconomyCog(commands.Cog, name="AdvEconomyCog"):
    ...
    async def add_money_role(self, ctx):
        ...
        list_users = [
            ...
            if role in user.roles and user.bot == False
        ]
        if len(list_users) == 1:
            self.bot.pyMongoManager.collection_profiles.find_one_and_update(
                {"user_id": list_users[0]}, {"$inc": {"cash": amount}}
            )
        elif len(list_users) == 0:
            await ctx.send("No hay usuarios con ese rol")
            return
        else:
            self.bot.pyMongoManager.collection_profiles.update_many(
                {"$or": {"user_id": list_users}}, {"$inc": {"cash": amount}}
            )
        await ctx.send(
            f"Se ha agregado {amount} :eggplant: a los usuarios con el rol `{role.name}`"
        )
    ...
    async def remove_money_role(self, ctx):
        ...
        if args[0].isdigit():
            if len(str(args[0])) == 18:
                ...
                await ctx.send(
                    'Uso correcto: `la!remove-money-role <role> <"all"/amount>`'
                )
                return
        else:
            if len(ctx.message.role_mentions) == 0:
        ...
        if allm:
            if len(list_users) == 0:
                ...
                self.bot.pyMongoManager.collection_profiles.find_one_and_update(
                    {"user_id": list_users[0]}, {"$set": {"cash": 0, "bank": 0}}
                )
            else:
                self.bot.pyMongoManager.collection_profiles.update_many(
                    {"$or": {"user_id": list_users}}, {"$set": {"cash": 0, "bank": 0}}
                )
            await ctx.send(
                f"Se les ha quitado todo el dinero a los usuarios con el rol {role.name}"
            )
        else:
            users_l = self.bot.pyMongoManager.get_profiles(list_users)
            for user in users_l:
                if amount > user["cash"]:
                    ...
                    user["cash"] -= amount
                self.bot.pyMongoManager.collection_profiles.find_one_and_replace(
                    {"user_id": user["user_id"]}, user
                )
                await ctx.send(
                    f"Se les ha quitado {amount} :eggplant: a los usuarios con el rol {role.name}"
                )
    ...
    async def reset_money(self, ctx):
        ...
        self.bot.pyMongoManager.collection_profiles.find_one_and_update(
            {"user_id": user_id}, {"$set": {"cash": 0, "bank": 0}}
        )
    ...
    async def reset_economy(self, ctx):
        ...
        await ctx.send(
            "¿Seguro que quieres eliminar el dinero de todos los usuarios? [Y/n]"
        )
        try:
