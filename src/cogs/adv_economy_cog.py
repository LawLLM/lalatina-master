...
class AdvEconomyCog(commands.Cog, name="AdvEconomyCog"):
    ...
    async def set_start_balance(self, ctx):
        ...
        if len(args) == 0:
            ...
            if args[0].isdigit():
                balance = int(args[0])
                self.bot.pyMongoManager.collection_guilds.update_one({'guild_id':512830421805826048}, {'$set': {'start_balance': balance}})
    ...
    async def add_money_role(self, ctx):
        ...
        if len(list_users) == 1:
            self.bot.pyMongoManager.collection_profiles.find_one_and_update({'user_id': list_users[0]}, {'$inc': {'cash': amount}})
        elif len(list_users) == 0:
            await ctx.send("No hay usuarios con ese rol")
            return
        else:
            self.bot.pyMongoManager.collection_profiles.update_many({'$or': {'user_id': list_users}}, {'$inc': {'cash': amount}})
    ...
    async def remove_money_role(self, ctx):
        ...
        if allm:
            if len(list_users) == 0:
                ...
                self.bot.pyMongoManager.collection_profiles.find_one_and_update({'user_id': list_users[0]}, {'$set': {'cash': 0, 'bank': 0}})
            else:
                self.bot.pyMongoManager.collection_profiles.update_many({'$or': {'user_id': list_users}}, {'$set': {'cash': 0, 'bank': 0}})
            ...
            for user in users_l:
                if amount > user['cash']:
                    ...
                    user['cash'] -= amount
                self.bot.pyMongoManager.collection_profiles.find_one_and_replace({'user_id': user['user_id']}, user)
    ...
    async def reset_money(self, ctx):
        ...
        self.bot.pyMongoManager.collection_profiles.find_one_and_update({'user_id': user_id}, {'$set': {'cash': 0, 'bank': 0}})
        ...
        try:
            ...
            if confirmation.content.lower() in ('y', 'n'):
                if confirmation.content.lower() == 'y':
                    self.bot.pyMongoManager.collection_profiles.update_many({}, {'$set': {'cash': 0, 'bank': 0}})
                else:
