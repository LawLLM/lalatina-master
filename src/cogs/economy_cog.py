...
class EconomyCog(commands.Cog, name="Economy"):
            ...
            if obj is not None:
                if obj["roleRequired"] is not None:
                    ...
                    if role not in ctx.author.roles:
                        await ctx.send(
                            f"Lo sentimos, pero no puedes comprar `{obj['name']}` si no tienes el rol `{role.name}`"
                        )
                        return
                if obj["stock"] == 0:
                    await ctx.send(
                        f"No queda `{obj['name']}` en la tienda {C_trencada}"
                    )
                    ...
                    if user["cash"] >= obj["value"]:
                        ...
                        if obj["stock"] is not None:
                            if obj["stock"] == 1:
                                self.bot.pyMongoManager.shop.delete_one(
                                    {"name": obj["name"]}
                                )
                                await ctx.message.channel.send(
                                    f"Ya no quedan más `{obj['name']}`"
                                )
                            else:
                                self.bot.pyMongoManager.shop.find_one_and_update(
                                    {"name": obj["name"]}, {"$inc": {"stock": -1}}
                                )
                                await ctx.message.channel.send(
                                    f"Número de `{obj['name']}` disponibles: `{obj['stock']-1}`"
                                )
                        ...
                        self.bot.pyMongoManager.collection_profiles.replace_one(
                            {"user_id": ctx.author.id}, user
                        )
    ...
    async def use(self, ctx):
        ...
        if (
            obj["name"] not in user["inventory"].keys()
            or user["inventory"][obj["name"]] == 0
        ):
            await ctx.send(f"No tienes `{obj['name']}` en el inventario")
            return
    ...
    async def additem(self, ctx):
        ...
        await ctx.send(
            "*Las preguntas opcionales se podrán saltar escribiendo `s`*\n*Pon `exit`  en cualquier momento para cancelar*"
        )
        ...
        try:
            roleAdded = await self.bot.wait_for(
                "message",
                check=lambda message: message.author == ctx.author,
                timeout=600,
            )
        ...
        try:
            roleRemoved = await self.bot.wait_for(
                "message",
                check=lambda message: message.author == ctx.author,
                timeout=600,
            )
        ...
        try:
            roleRequired = await self.bot.wait_for(
                "message",
                check=lambda message: message.author == ctx.author,
                timeout=600,
            )
            ...
            if args[1].isdigit():
                ...
                if member.bot:
                    ...
                    if amount == 0:
                        ...
                        await ctx.send(
                            f"No tienes suficiente dinero. Ahora mismo tienes **{player_author['cash']}** :eggplant: "
                        )
                    else:
                        new_balance_author = player_author["cash"] - amount
                        new_balance_receiver = player_receiver["cash"] + amount

                        self.bot.pyMongoManager.update_money(
                            ctx.author.id, new_balance_author
                        )
                        self.bot.pyMongoManager.update_money(
                            member.id, new_balance_receiver
                        )
                ...
                await ctx.send(
                    "Cantidad no válida. Uso correcto: `la!givemoney <user> <amount>`"
                )
        else:
            await ctx.send("Uso correcto: `la!givemoney <user> <amount>`")
            ...
            if len(args) == 2:
                if args[1].isdigit():
                    if len(ctx.message.mentions) > 0:
                        ...
                        if len(args[0]) != 18:
                            await ctx.send(
                                "Uso correcto: `la!addmoney <user> <amount>`"
                            )
                            return
                        try:
                            member = self.bot.get_user(int(args[0]))
                    ...
                    await ctx.send(
                        f"Dinero de {member.name}: **{new_balance}** :eggplant:"
                    )
                else:
            ...
            if len(args) == 2:
                if args[1].isdigit():
                    if len(ctx.message.mentions) > 0:
                        ...
                        if len(args[0]) != 18:
                            await ctx.send(
                                "Uso correcto: `la!removemoney <user> <amount>`"
                            )
                            return
                        try:
                            member = self.bot.get_user(int(args[0]))
    ...
    async def top(self, ctx):
        ...
        list_result.sort(
            key=lambda result: result["cash"] + result["bank"], reverse=True
        )
    ...
    async def addphrase(self, ctx):
        if ctx.message.author.guild_permissions.administrator:
            ...
            if len(args) == 0:
                await ctx.send(
                    "Debes poner la frase, sustituyendo las ganancias por `{{{}}}`".format(
                        "amount"
                    )
                )
                ...
                if "{{{}}}".format("amount") in args:
                    self.bot.pyMongoManager.add_work_phrase(
                        " ".join(args).replace("🍆", ":eggplant:")
                    )
                    await ctx.send("Frase añadida!")

                else:
                    await ctx.send(
                        "Debes sustituir las ganancias por `{{{}}}`".format("amount")
                    )
            ...
            if len(args) == 0:
                ...
                if len(args) == 2 and args[0].isdigit() and args[1].isdigit():
                    ...
                    try:
                        ...
                        self.bot.pyMongoManager.collection_guilds.update(
                            {"guild_id": 512830421805826048},
                            {"$pull": {"work_phrases": {"$in": phrase}}},
                            upsert=False,
                            multi=True,
                        )
                    ...
                    try:
                        ...
                        self.bot.pyMongoManager.collection_guilds.update(
                            {"guild_id": 512830421805826048},
                            {"$pull": {"work_phrases": phrase}},
                        )
                        await ctx.send("Frase eliminada!")
                    except IndexError:
                        await ctx.send(
                            f"El índice es incorrecto (No existe elemento nº{index}"
                        )
    ...
    async def withdraw(self, ctx):
        ...
        usr = self.bot.pyMongoManager.collection_profiles.find_one(
    ...
    @commands.command(
        aliases=["remove-item", "delete-item", "remove-object", "delete-object"]
    )
    ...
    async def edit_item(self, ctx):  # la!edit-item <opcion> <valor> <nombre>
        ...
        if args[0].lower() not in (
            ...
            await ctx.send(
                f"Parámetros editables: `name` `value` `description` `image` `message` `stock` `roleAdded`, `roleRemoved`, `roleRequired` "
            )
            return
        ...
        if args[1].isdigit() and args[0].lower() in (
            "stock",
            "value",
            "roleadded",
            "roleremoved",
        ...
        self.bot.pyMongoManager.collection_shop.find_one_and_update(
            {"key": cons}, {"$set": {args[0]: args[1]}}
        )
