...
class RewardsCog(commands.Cog, name="Rewards"):
    ...
    async def daily(self, ctx):
        ...
        if gmtime_previus.tm_year < gmtime_current.tm_year - 1:
            ...
            if gmtime_previus.tm_mon == 12 and gmtime_previus.tm_mday == 31 and gmtime_current.tm_yday == 1:
                multiplier += 1
            else:
                multiplier = 1
        else:
        ...
        if not can_claim:
            time_advanced_seconds = gmtime_current.tm_hour*3600+gmtime_current.tm_min*60+ gmtime_current.tm_sec
